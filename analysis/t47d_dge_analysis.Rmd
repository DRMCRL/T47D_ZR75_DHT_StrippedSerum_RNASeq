...
write_lines(
  nonZero,
  here::here("output/T47D_StrippedSerum_nonZero.txt")
)
```

For the purposes of data integration, the `r comma(length(nonZero))` genes with one or more count anywhere were also identified as a set of the most inclusive genes for expression.
...
topTable %>%
  ...
  geom_text_repel(
    ...
    data = . %>%
      dplyr::filter(DE, logFC > 1.4 | (logFC < -1 & logCPM > 5)),
    show.legend = FALSE
  ) +
