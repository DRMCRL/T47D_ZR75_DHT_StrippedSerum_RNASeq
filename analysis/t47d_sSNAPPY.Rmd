...
dge <- here::here("output/T47D_dge.rds") %>%
  read_rds()
logCPM <- read_tsv(here::here("output/T47D_DHT_StrippedSerum_RNASeq_logCPM.tsv"))
logCPM <- logCPM %>%
...
# )
# write_rds(permutedScore, here::here("output/t47d_permutedscore.rds"))
permutedScore <- read_rds(here::here("output/t47d_permutedscore.rds"))
```

Becusae the samll number of samples, the null distributions are close to but not exactly normally distribution. 
...
Average treatment effect on each pathway was modelled using a `z test`. There wasn't any pathway that passed the statistical threshold (ie. FDR < 0.05). 
