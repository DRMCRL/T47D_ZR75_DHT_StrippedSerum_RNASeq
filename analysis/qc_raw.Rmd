...
```{r plotSummary2, fig.height=fh, fig.cap = "*Overall summary of FastQC reports*"}
plotSummary(rawFqc)
```
...
```{r plotReadTotals2, fig.height=fh, fig.cap = "*Library Sizes for all supplied fastq files. Any samples run as multiple libraries are  shown as the supplied multiple libraries and have not been merged.*"}
plotReadTotals(rawFqc, pattern = suffix, usePlotly = TRUE)
```

### Sequence Quality

```{r plotBaseQuals2, fig.height=fh, fig.cap = "*Mean sequencing quality scores at each base position for each library*"}
plotBaseQuals(
  rawFqc,
  pattern = suffix, 
  usePlotly = TRUE,
...
```{r plotGC2, fig.height=fh, fig.cap=glue("*GC content shown as the % above and below the theoretical GC content for the {sp} transcriptome.*")}
plotGcContent(
  x = rawFqc, 
  pattern = suffix, 
  species = sp, 
...
```{r plotCumulativeGC2, fig.cap = "*GC content shown as a cumulative distribution for all libraries. Groups can be hidden by clicking on them in the legend.*"}
ggplotly(
  getModule(rawFqc, "Per_sequence_GC_content") %>%
    group_by(Filename) %>%
    mutate(
...
```{r plotSeqContentResid2, fig.cap = "*Base and Position specific residuals for each sample. The mean base content at each position was calculated for each nucleotide, and the sample-specific residuals calculated.*"}
plotly::ggplotly(
  getModule(rawFqc, module = "Per_base_sequence_content") %>% 
    mutate(Base = fct_inorder(Base)) %>%
    group_by(Base) %>% 
...
```{r plotAdapterContent2, fig.height=fh, fig.cap="*Total Adapter Content for each sample shown by starting position in the read.*"}
plotAdapterContent(
  x = rawFqc, 
  pattern = suffix, 
  usePlotly = TRUE,
...
```{r plotOverRep2, fig.height=os_fh, fig.cap = "*Summary of over-represented sequences across all libraries*"}
if (nrow(os)){
  if (length(unique(os$Filename)) > 1){
    suppressMessages(
      plotOverrep(
