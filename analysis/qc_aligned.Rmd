...
```{r alnStats}
alnFiles <- here::here() %>%
  list.files(recursive = TRUE, pattern = "Log.final.out", full.names = TRUE) 
alnStats <- alnFiles %>%
  lapply(function(x){
  ...
  bind_rows() %>%
  mutate(
    Run = str_extract(Filename, runRegEx),
...
ggplotly(
  alnStats %>%
    ...
    separate(Filename, c("Run", "Filename"), sep = "/") %>%
...
countSummary <- here::here("data/aligned/counts/counts.out.summary") %>%
  ...
  mutate(
    Run = str_extract(Sample, runRegEx),
    Total = rowSums(
...
counts <- here::here("data/aligned/counts/merged_counts.out") %>%
  read_tsv(comment = "#") %>%
  rename_all(basename) %>%
  dplyr::select(Geneid, starts_with("GLL")) %>%
...
pca %>%
  autoplot(
    data = .$x %>%
      rownames() %>%
      enframe(value = "sample", name = c()) %>%
      left_join(samples),
    colour = "group",
    label = showLabel, label.repel = showLabel, label.show.legend = FALSE
  ) +
  labs(colour = "Group") +
  scale_colour_manual(values = group_cols)
```
...
```{r plotPCA_GC_Length, fig.height=6, fig.cap = "*Contribution of each GC/Length Bin to PC1 and PC2. Fill colours indicate the t-statistic, with tranparency denoting significance as -log10(p), using Bonferroni-adjusted p-values. GIven that the cell-line is clearly captured by PC1, the appearance of GC & length artefacts on PC2 is as expected.*"}
mcols(genesGR) %>%
  as.data.frame() %>%
